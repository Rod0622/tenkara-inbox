...
export default function Sidebar({
  ...
  // Phase 3: track which folders the user has expanded to show All / Closed sub-views.
              ...
              {isExpanded && (
                <div className="ml-5 pl-2 border-l border-[#1E242C] mt-0.5 mb-1">
                  {accountFolders.map((folder) => {
                    ...
                    // Per spec: clicking folder name = open + unassigned + in this folder.
                    // Inbox is special-cased because legacy unfoldered convos with no folder_id
                    // historically lived in Inbox view.
                    let folderUnreadCount = 0;
                    let folderClickCount = 0; // matches what clicking would show
                    if (isSystemInbox) {
                    ...
                    const handleFolderClick = () => {
                      ...
                      setActiveView("inbox");
                      if (setFolderSubView) setFolderSubView("unassigned");
                    };
                    ...
                    return (
                      <div key={folder.id}>
                        <div className="flex items-center group/folder">
                          {/* Expand/collapse caret to show sub-views */}
                          <button
                            onClick={toggleFolderExpand}
                            className="w-4 h-5 flex items-center justify-center text-[#484F58] hover:text-[#7D8590] shrink-0"
                            aria-label={isFolderExpanded ? "Collapse" : "Expand"}
                          >
                          ...
                          >
                            ...
                            <span className="flex items-center gap-1 shrink-0">
                              ...
                              {folderClickCount > 0 && (
                                <span className="text-[10px] text-[#484F58]">{folderClickCount}</span>
                              )}
                            </span>
                          </button>
                        ...
                        {/* Sub-views: All + Closed (only when expanded) */}
                        {isFolderExpanded && (
                          <div className="ml-7 mt-0.5 mb-0.5 flex flex-col gap-0.5">
                            <button
                              onClick={() => {
                                window.location.hash = "";
                                setActiveFolder(folder.id);
                                setActiveMailbox(mb.id);
                                setActiveView("inbox");
                                if (setFolderSubView) setFolderSubView("all");
                              }}
                              className={`text-left px-2 py-0.5 rounded text-[11px] transition-colors ${
                                isFolderActive && folderSubView === "all"
                                  ? "bg-[#1E242C] text-[#E6EDF3]"
                                  : "text-[#484F58] hover:bg-[#12161B] hover:text-[#7D8590]"
                              }`}
                            >
                              All
                            </button>
                            <button
                              onClick={() => {
                                ...
                                setActiveMailbox(mb.id);
                                setActiveView("inbox");
                                if (setFolderSubView) setFolderSubView("closed");
                              }}
                              className={`text-left px-2 py-0.5 rounded text-[11px] transition-colors ${
                                isFolderActive && folderSubView === "closed"
                                  ? "bg-[#1E242C] text-[#E6EDF3]"
                                  : "text-[#484F58] hover:bg-[#12161B] hover:text-[#7D8590]"
                              }`}
                            >
