...
// Each paragraph is followed by an empty <p></p> to give visible breathing room
// between paragraphs (mirrors how a person would space their email).
  ...
  return (
    <div className="flex-1 flex flex-col bg-[var(--bg)] overflow-hidden">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[13px] font-bold transition-all ${
              !sending && (to.length > 0 || toInput.trim()) && subject.trim()
                ? "bg-[var(--accent)] text-[var(--bg)] hover:bg-[var(--accent-strong)] active:scale-[0.98]"
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {/* From */}
        <div className="px-5 py-2.5 border-b border-[var(--border)] flex items-center gap-3">
                ...
                >
                  {accounts.map((acc) => (
                    <button
                      ...
                      className={`flex items-center gap-2 w-full px-3 py-2 text-[12px] hover:bg-[var(--surface-hover)] transition-colors ${
        ...
        {/* To — tag style */}
        <div className="px-5 py-2 border-b border-[var(--border)] flex items-center gap-3">
          <span className="text-[12px] font-semibold text-[var(--text-muted)] w-12 shrink-0">To</span>
          <div className="flex-1 flex flex-wrap items-center gap-1">
            {to.map((email, i) => (
              <span key={i} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[var(--surface-hover)] text-[11px] text-[var(--text-primary)]">
                {email}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowTemplateModal(false)}>
          <div className="w-full max-w-lg bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="px-5 py-3 border-b border-[var(--border)] flex items-center justify-between">
              ...
              <button onClick={() => setShowTemplateModal(false)} className="w-7 h-7 rounded-md text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-hover)] flex items-center justify-center">
                <X size={16} />
              </button>
            </div>
                      ...
                      <div key={scope}>
                        ...
                        {scopeTemplates.map((tpl) => (
                          <button key={tpl.id} onClick={() => insertTemplate(tpl)}
                            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-[var(--surface-hover)] text-left transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowDriveModal(false)}>
          <div className="w-full max-w-md bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="px-5 py-3 border-b border-[var(--border)] flex items-center justify-between">
              ...
              <button onClick={() => setShowDriveModal(false)} className="w-7 h-7 rounded-md text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-hover)] flex items-center justify-center">
                <X size={16} />
              </button>
            </div>
                ...
                <div className="space-y-0.5">
                  {driveFolders.map((f) => (
                    <button key={f.id} onClick={() => navigateDriveFolder(f)}
                      className="w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-[var(--surface-hover)] text-left transition-colors">
