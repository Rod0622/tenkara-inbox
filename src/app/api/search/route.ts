...
export async function GET(req: NextRequest) {
  ...
  for (let i = 0; i < idArray.length; i += 50) {
    ...
    let fetchQuery = supabase
      .from("conversations")
      .select(`id, email_account_id, folder_id, thread_id, subject, from_name, from_email, preview, is_unread, is_starred, assignee_id, status, has_attachments, attachment_count, last_message_at, created_at, updated_at, supplier_contact_id, supplier_contact:supplier_contacts(id, name, company), primary_contact_name, primary_contact_email, primary_contact_is_manual, assignee:team_members!conversations_assignee_id_fkey(*), labels:conversation_labels(label_id, label:labels(*))`)
      .in("id", batch)
      .neq("status", "trash");
