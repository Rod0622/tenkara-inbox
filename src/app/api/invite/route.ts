...
function inviteEmailHtml({ acceptUrl, invitedBy }: { acceptUrl: string; invitedBy?: string }) {
  return `
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:560px;margin:0 auto;padding:40px 20px;">
      ...
      <p style="font-size:15px;color:#4B5563;text-align:center;margin-bottom:28px;line-height:1.6;">
        ${invitedBy || "An admin"} invited you to access the shared inbox app.
      </p>
...
export async function POST(req: NextRequest) {
  ...
  if (Array.isArray(email_account_ids) && email_account_ids.length > 0) {
    ...
    const { error: accessErr } = await supabase.from("account_access").insert(rows);
    if (accessErr) return NextResponse.json({ error: accessErr.message }, { status: 500 });
  }

  const acceptUrl = `${getAppUrl()}/accept-invite/${inviteToken}`;

  const sent = await sendTransactionalEmail({
    to: normalizedEmail,
    subject: "You've been invited to Tenkara Inbox",
    html: inviteEmailHtml({ acceptUrl, invitedBy: invited_by }),
  });
  ...
  return NextResponse.json({
    ...
    message: sent.ok
      ? `Invitation sent to ${normalizedEmail} from ${sent.senderEmail || "Tenkara"}`
